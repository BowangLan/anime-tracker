      ...
      <ol className="border-t border-[var(--fr-hairline-soft)] xl:min-h-0 xl:flex-1 xl:overflow-y-auto xl:overscroll-contain xl:pr-1">
        {popular.map(({ anime: show, airing }, index) => (
          <li key={show.id} className="border-b border-[var(--fr-hairline-soft)]">
            <Link href={animePath(show.id, show.title)} className="group grid grid-cols-[22px_46px_minmax(0,1fr)_16px] items-center gap-3 py-3 outline-none transition hover:bg-[var(--fr-surface-1)] focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-[var(--fr-accent-blue)]">
